...
строкой `TODO: удалить перед запуском` — `grep -rn "удалить перед запуском" src scripts`
находит всё разом.

### Выключателя нет — только удаление

Переменной `DEMO_MODE` больше не существует: по прямой просьбе демо-функции
включены всегда и настройки в Vercel не требуют. Это упрощает показ и
переносит всю ответственность на удаление кода перед запуском.

Что это значит буквально, чтобы потом не было сюрприза: **на боевом сайте
любой, кто зарегистрируется, увидит в углу плашку «Посетитель · Модератор» и
одним нажатием получит права модератора** — чужие анкеты, кнопки блокировки.
Раньше это закрывала переменная; теперь не закрывает ничего. Пока настоящих
пользователей нет, цена нулевая; в день, когда придёт первый посторонний,
код уже должен быть удалён.

Убирается всё по списку файлов в конце раздела — и ни одна строка из этого
списка не удаляется «настройкой». Пункт 4 к тому же живёт в схеме базы и
требует отдельного отката миграцией.

### 0. Витрина без базы

**Сайт показывает все функции, даже когда PostgreSQL не подключён вовсе.**
Настраивать для этого нечего: если переменных базы в окружении нет, страницы
берут выдуманные данные из `src/lib/demo/showcase.ts` через
`src/lib/demo/fallback.ts`.

Что именно работает без базы:

| Где | Что видно |
|---|---|
| магазин | оценки товаров, фильтр по оценке, отзывы на карточке |
| карточка товара | чат с менеджером поставщика — ответы заготовлены, живут до перезагрузки |
| «Бригады» | семь бригад с рейтингом, завершёнными заявками и отзывами |
| плашка в углу | «Посетитель · Модератор» — видна всем, без входа |
| `/moderation` | очередь из семи человек; блокировка и одобрение работают в браузере |

Один и тот же список данных питает и витрину без базы, и сид-скрипт
(`scripts/seed-demo.ts`): иначе две копии «примерно одинаковых» отзывов
разошлись бы после первой правки, и человек видел бы разное до и после
подключения базы.

Три вещи, за которые здесь заплачено осознанно:

- **Ничего не сохраняется.** Заблокированный человек разблокируется
  перезагрузкой, отправленное сообщение исчезает. Так и написано на самих
  экранах: сделать вид, что решение записано, было бы хуже пустой страницы —
  человек ушёл бы уверенным, что кого-то заблокировал.
- **`/moderation` открыт всем.** Без базы нет ни сессий, ни ролей, и запирать
  нечего: все настоящие данные лежат в базе, которой нет. Как только
  переменные заданы, проверка роли работает как раньше — это проверено
  отдельно.
- **Даты у отзывов фиксированные.** Считать их от «сегодня» нельзя: собранная
  страница и браузер получили бы разные значения, и React сообщил бы о
  расхождении разметки.

Как только база подключена, вся эта ветка выключается сама и данные идут из
неё — смешивания выдуманного с настоящим не происходит ни секунды.

### 0.1. Автоматический вход гостем

Посетитель без cookie молча получает учётную запись и сессию —
`POST /api/demo/guest`, вызываемый из `DemoAutoGuest` в корневой разметке.
Именно это делает всё остальное видимым без регистрации.

Так, а не «снять проверки входа», по одной причине: переписка, отзывы и смена
роли упираются не в интерфейс, а в базу. Сообщение без автора не пропустит
триггер, отзыв без `author_id` не вставится. Снимать пришлось бы правила в
схеме — то есть ломать ровно то, ради показа чего всё и затевалось. Здесь не
ослаблено ничего: у гостя просто появилось имя.

Запрос уходит из браузера, а не из `proxy.ts`, который тоже видит каждый
заход: proxy выполняется до отдачи страницы, включая главную и магазин,
собранные заранее и лежащие на CDN. Ходить оттуда в базу — значит сделать
готовые страницы вычисляемыми на каждый заход. Здесь же запрос идёт один раз,
уже после показа.

Один раз за вкладку — за это отвечает флаг рядом с модулем, а не состояние
компонента: React в строгом режиме монтирует дерево дважды, и без флага
первый заход заводил бы двух гостей.

⚠️ Цена: строка в `users` на каждого посетителя, включая поисковых роботов, и
приветственное уведомление к ней. Плюс главное — в связке с пунктами 2 и 5
права модератора получает любой, кто открыл сайт. Убирается только удалением
кода, см. TODO_BEFORE_LAUNCH.md, пункт 7.
...
В кабинете — плашка с кнопками «Заказчик ·
...
Выключателя у маршрута нет: закрытый, он не работал
...
Плашка снизу слева **на любой странице**, а не только
в кабинете. Видна каждому вошедшему, без условий. Тот же маршрут смены роли, что в пункте 2.
...
| Файл | Пункт |
|---|---|
| `src/lib/demo/` целиком + ветки `!isDbConfigured()` в маркете, «Услугах», модерации и `src/proxy.ts` | 0 |
| `src/app/api/demo/guest/`, `src/components/demo/DemoAutoGuest.tsx` + две строки в `src/app/layout.tsx` | 0.1 |
...
**до** сборки. Вторым шагом идёт `npm run db:seed:deploy` — он заводит
демо-данные, если их ещё ни разу не заводили (см. ниже).

```json
"vercel-build": "npm run db:migrate:deploy && npm run db:seed:deploy && next build"
```
...
### Демо-данные заводятся тем же деплоем (⚠️ временно)

> Это часть **временной вставки №1** из [`TODO_BEFORE_LAUNCH.md`](TODO_BEFORE_LAUNCH.md).
> Перед настоящим запуском шаг убирается вместе с самими демо-данными.

Второй шаг `vercel-build` — `npm run db:seed:deploy`. Он запускает
`scripts/seed-demo.ts --if-absent`: заводит тестовых людей, бригады, отзывы
и менеджеров, чтобы витрина не была пустой, — и ничего для этого не нужно
открывать, вводить или нажимать.

Пропускает он себя по тем же трём условиям, что и миграции (нет базы,
`VERCEL_ENV` не `production`, аварийная переменная — здесь
`SKIP_DEPLOY_SEED=1`), плюс по четвёртому, главному.

**Отметка, а не проверка «есть ли данные».** Заведя данные, скрипт пишет
строку в таблицу `demo_seed_state` (миграция `0016`) — одну-единственную,
`check (id = true)` не даст появиться второй. Следующий деплой видит отметку
и молча уходит.

Отметка нужна именно потому, что «посмотреть, есть ли демо-люди» — плохой
признак. Убрали демо-данные перед запуском — людей нет, и деплой завёл бы их
обратно. С отметкой уборка остаётся уборкой: `--clean` данные удаляет,
отметку оставляет, и деплой больше к этой теме не возвращается.

Обратная сторона того же: база, где данные уже есть, а отметки ещё нет (их
заводили руками до этой доработки), не чистится и не дублируется — деплой
просто ставит отметку `«найдены готовые данные»` и не трогает ничего.

**В режиме `--if-absent` скрипт никогда не удаляет.** При ручном запуске
`npm run demo:seed` он сначала сносит прежние демо-данные, чтобы не
спотыкаться об уникальные индексы; на деплое такой уборки нет — деплой не
имеет права трогать то, чего не заводил.

**Упавший засев сборку не валит** — в отличие от миграций. Разница
намеренная: схема — это условие работоспособности кода, а витринные отзывы —
украшение. Красная сборка из-за демо-отзывов остановила бы выкатку
настоящих исправлений, и цена ошибки была бы больше самой ошибки. Поэтому в
логе будет предупреждение, а деплой пойдёт дальше.
...
шаг миграций (и засева) молча пропустится, а приложение будет работать без
базы.

Необязательные аварийные выключатели: `SKIP_DEPLOY_MIGRATIONS=1` отключает
миграции, `SKIP_DEPLOY_SEED=1` — засев демо-данными.
...
migrations first, then seeds demo data once — см. раздел
[«Миграции при деплое»](#миграции-при-деплое) выше.
