...
Четыре бригады, заказчик и модератор; у бригад разное число отзывов — 4, 2, 1
и ни одного, чтобы было видно и «★ 4,8 · 4 отзыва», и «пока нет отзывов» у
новичка. Пароль у всех `Demo-12345`, почта заканчивается на
`@demo.opusgroup`, тексты отзывов начинаются с `[демо]` — по этим меткам они
и находятся.

| Учётная запись | Роль |
|---|---|
| `client@demo.opusgroup` | заказчик |
| `moderator@demo.opusgroup` | модератор |
| `krovlya-urala@demo.opusgroup` | исполнитель, ★ 4,8 |
| `fasad-plyus@demo.opusgroup` | исполнитель, ★ 3,5 |
| `mastera-prikamya@demo.opusgroup` | исполнитель, ★ 5,0 по одному отзыву |
| `novaya-brigada@demo.opusgroup` | исполнитель без отзывов |
