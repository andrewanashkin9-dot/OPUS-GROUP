...
- Выплат исполнителям: `transactions` знает, сколько кому причитается, но
  деньги никуда не уходят.
- Подтверждения почты, восстановления пароля, входа по телефону.
- Экрана модерации и профиля исполнителя (специализации, рейтинг) — бригады
  всё ещё захардкожены в `src/lib/crews.ts`.
