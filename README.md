...
писал» разбирается по строкам, которые нельзя переписать.
...
| | |
|---|---|
| переведено | главная (вместе с тарифами — это её раздел), бригады, шапка, подвал, переключатель темы |
| осталось по-русски | кабинет, формы входа и регистрации, магазин, конструктор, база знаний |
...
Тем же скриптом заводятся отзывы о товарах — те же случаи, что и у бригад:

| Товар | Рейтинг | Отзывов |
|---|---|---|
| `tn-shinglas-ultra` | ★ 4,7 | 9 |
| `tarkett-holiday-laminate` | ★ 4,3 | 3 |
| `rockwool-scandic` | ★ 3,8 | 5 |
| `rehau-blitz-window` | ★ 3,0 | 2 (полярные: 1 и 5) |
| `docke-block-house` | ★ 2,0 | 4 |
| `braer-brick-red` | ★ 5,0 | 1 |

Остальные тридцать одна позиция каталога остаются без отзывов — так и
должно быть: это показывает, как выглядит карточка, которую ещё никто не
оценил.
...
| `src/app/api/demo/` | 2 |
| `src/components/demo/` | 2 |
| `src/lib/demo-mode.ts` | 1, 2 |
| `DEMO_MODE` в `.env.example` | 1, 2 |
