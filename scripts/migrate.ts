...
async function main(): Promise<void> {
  ...
  try {
    // Замок на всё время работы.
    //
    // Нужен с тех пор, как миграции применяются при деплое: два деплоя
    // подряд (поправил и сразу запушил ещё раз) запускают два сборщика, и
    // оба видят одну и ту же неприменённую миграцию. Без замка второй
    // начинает применять её, пока первый ещё не отметил в
    // schema_migrations, — и файл выполняется дважды. Для `create table`
    // это ошибка и красная сборка, для `insert` — молча удвоенные данные,
    // что хуже.
    //
    // Advisory lock, а не таблица с флагом: его снимает сама база, когда
    // соединение обрывается. Флаг в таблице после упавшего сборщика
    // остался бы стоять навсегда, и разбирать это пришлось бы руками в
    // худший момент — когда деплой не идёт.
    //
    // Число — просто идентификатор замка; никакого смысла, кроме «этот
    // замок наш», в нём нет.
    const LOCK_ID = 8_140_271;
