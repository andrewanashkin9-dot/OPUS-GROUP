 ...
 *
 * ⚠️ Показывается **при каждом заходе** — так попросили. Изначально было
 * ровно один раз, после регистрации, и хранителем этого «одного раза»
 * служила строка в notifications.
 *
 * «Каждый заход» здесь означает каждую полную загрузку страницы, а не
 * каждый переход внутри сайта: компонент живёт в корневой разметке и при
 * внутренней навигации не пересоздаётся. Иначе карточка выскакивала бы на
 * каждый клик по меню.
 *
 * Прочитанным приветствие всё равно помечается — один раз, при первом
 * показе. Не ради «одного раза», а ради колокольчика: непрочитанное
 * приветствие висело бы в счётчике вечно.
 *
...
interface Welcome {
  /** null — приветствия в базе нет, текст собран из роли на лету. */
  id: string | null;
  text: string;
}
    ...
    void apiJson<{ welcome: Welcome | null; role: string }>("/api/notifications/welcome").then(
      (result) => {
        ...
        timer = setTimeout(() => {
          ...
          setWelcome(found);
          setStep(target);
          // Гасим счётчик колокольчика. Повторный вызов ничего не делает:
          // маршрут обновляет только непрочитанные. У приветствия, собранного
          // на лету, id нет и гасить нечего.
          if (found.id) void apiJson(`/api/notifications/${found.id}/read`, { method: "POST" });
        }, APPEAR_DELAY_MS);
      },
    );
