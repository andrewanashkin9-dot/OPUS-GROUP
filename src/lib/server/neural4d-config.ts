...
/**
 * Сколько вариантов вендор строит за один запрос — и почему мы этим пока
 * не управляем.
 *
 * По умолчанию их четыре: настоящий ответ вернул четыре номера заданий и
 * списал 120 баллов. Нужен один, но имя параметра неизвестно.
 *
 * Опыт, чтобы его найти, поставлен и провалился. Идея была такая: послать
 * запрос без файла — он отбивается валидацией до списания — и приложить
 * десяток кандидатов (`num`, `count`, `modelNum`, `batchSize`, …) с
 * заведомо негодными значениями. Валидатор, который перечисляет все
 * непрошедшие поля, тем самым назвал бы существующие. Контролем служило
 * `faceNum`: оно точно есть, вендор сам вернул его в `generationConfig`.
 *
 * Ответ пришёл ровно один: «Image file is required». Ни слова про faceNum.
 * Значит, вендор останавливается на первой же ошибке, и узнать имена полей
 * бесплатным запросом нельзя вовсе — ни этих, ни каких-либо ещё.
 *
 * Отсюда правило: не подбирать имя перебором в коде. Следующий шаг —
 * документация вендора, к которой есть доступ у владельца аккаунта.
 * Догадка здесь стоит 120 баллов за попытку, и мы уже теряли время на
 * выдуманном `/reconstruct`.
 */
